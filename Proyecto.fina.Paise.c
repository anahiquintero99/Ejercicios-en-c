...
int grabar()
{
	if((esc=fopen("paises_boletas.txt","w"))==NULL)
	{
		printf("NO puedo abrir el archivo...");
	}
	else{
		for (i=0;i<50;i++){
			fprintf(esc,"\t\t\t\t IPN\n\t\t\t\tUPIICSA\n");
			fprintf(esc,"\t\t\t\t\t\t\t\tFOLIO: %d\n\n",i);
			fprintf(esc,"\t Nombre Pais: %s \t BOLETA:%d \n",pais[i], nb[i]);
			fprintf(esc,"\t INFORMACIÓN SOBRE EL PAIS\n\n");
			fprintf(esc,"\t Nombre del Presidente: %s %s %s \n",nombrep[i],app[i],amp[i]);
			fprintf(esc,"\t Poblacion: \t\t\t\t %d millones\n",poblacion[i]);
			fprintf(esc,"\t Estados: \t\t\t\t %d estados\n",estado[i]);
			fprintf(esc,"\t Iidioma: \t\t\t\t %s\n",idioma[i]);
			fprintf(esc,"\t Gentilicio: \t\t\t\t %s\n",gentilicio[i]);
			fprintf(esc,"\t Capital: \t\t\t\t %s\n",capital[i]);
			fprintf(esc,"__________________________________________________________________________________\n\n");
			}
		}
}
